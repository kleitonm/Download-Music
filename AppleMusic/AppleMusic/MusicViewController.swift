...
class MusicViewController: UIViewController {
    ...
    func downloadTapped(_ sender: UIButton) {
        for url in urlDownloads {
            fownloadService.startDownload(URL(string: url)!)
        }
    }
    ...
    func playDownload(_ sebder: UIButton) {
        ...
        present(playerViewController,m animated: true, completion: nil)
    ...
    let documentsPAth = FileManager.default.url(for: .documentDirectory, in: .userDomainMask).first!
    func localFilePath(for url: URL) -> URL {
        return documentsPAth.appendingPathComponent(url.lastPathComponent)
    }
}

extension MusicViewController: ViewController {
